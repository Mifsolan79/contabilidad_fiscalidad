...
AO = ${e} / ${p} = ${$(m,4)}`,correctValue:m,valueType:"number"})}return o},q=t=>{const o=[],s=["liquidez","tesoreria","disponibilidad","garantia","endeudamiento","calidad_deuda"];for(let n=0;n<t;n++){const a=s[n%s.length],e=l(100,1e4),i=l(10,50)*e,d=l(20,80)*e,c=l(30,100)*e,p=i+d+c,m=l(100,500)*e,u=p+m,y=Math.floor(u/l(120,250)*100),A=u-y,b=Math.floor(y*(l(30,80)/100));let g="",C="",h="",P={},M="",v=0;switch(a){case"liquidez":M="Ratio de Liquidez General",g="Calcula el Ratio de Liquidez General.",P={"Activo Corriente":r(p),"Pasivo Corriente":r(b)},v=p/b,C=$(v,2),h=`Fórmula: AC / PC = ${p} / ${b} = ${v.toFixed(4)}`;break;case"tesoreria":M="Ratio de Tesorería",g="Calcula el Ratio de Tesorería (Acid Test).",P={Realizable:r(d),Disponible:r(i),"Pasivo Corriente":r(b)},v=(d+i)/b,C=$(v,2),h=`Fórmula: (Realizable + Disponible) / PC = (${d} + ${i}) / ${b} = ${v.toFixed(4)}`;break;case"disponibilidad":M="Ratio de Disponibilidad",g="Calcula el Ratio de Disponibilidad.",P={Disponible:r(i),"Pasivo Corriente":r(b)},v=i/b,C=$(v,2),h=`Fórmula: Disponible / PC = ${i} / ${b} = ${v.toFixed(4)}`;break;case"garantia":M="Ratio de Garantía",g="Calcula el Ratio de Garantía.",P={"Activo Total":r(u),"Pasivo Total":r(y)},v=u/y,C=$(v,2),h=`Fórmula: Activo Total / Pasivo Total = ${u} / ${y} = ${v.toFixed(4)}`;break;case"endeudamiento":M="Ratio de Endeudamiento",g="Calcula el ratio de endeudamiento total.",P={"Pasivo Total":r(y),"Patrimonio Neto":r(A)},v=y/(A+y),C=$(v,2),h=`Fórmula: Pasivo / (PN + Pasivo) = ${y} / (${A} + ${y}) = ${v.toFixed(4)}`;break;case"calidad_deuda":M="Ratio de Calidad de la Deuda",g="Calcula el ratio de Calidad de la Deuda.",P={"Pasivo Corriente":r(b),"Pasivo Total":r(y)},v=b/y,C=$(v,2),h=`Fórmula: Pasivo Corriente / Pasivo Total = ${b} / ${y} = ${v.toFixed(4)}`;break}o.push({id:f("RATIO",n),category:"Ratios Financieros y Patrimoniales",title:M,question:g,data:P,solution:C,explanation:h,correctValue:v,valueType:"number"})}return o},I=t=>{const o=[];for(let s=0;s<t;s++){const n=Math.random()>.5,a=l(100,1e3)*1e3,e=Math.floor(a*F(.4,.7)),i=Math.floor(a*F(.8,1.5)),d=Math.floor(i*F(.1,.25)),c=Math.floor(d*F(.1,.3)),m=Math.floor((d-c)*(1-.25));if(n){const u=d/a*100;o.push({id:f("RENT",s),category:"Rentabilidad",title:"Rentabilidad Económica (ROI)",question:"Calcula el ROI (%).",data:{"Activo Total":r(a),BAII:r(d)},solution:`${$(u)} %`,explanation:`Fórmula: (BAII / Activo) x 100
