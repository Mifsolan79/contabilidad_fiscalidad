...
FM = (${c} + ${u}) - ${s} = ${p}`,correctValue:p,valueType:"currency"})}}return o},k=e=>{const o=[];for(let n=0;n<e;n++){const t=d(500,5e3)*100,i=Math.floor(t*(d(40,70)/100)),a=t-i,s=Math.floor(a*.9),r=Math.floor(a*.2),c=d(r/100,s/100)*100,u=a-c,g=a/u,p=["Calcula el Grado de Apalancamiento Operativo (GAO) para esta empresa.","Determina la sensibilidad del beneficio ante cambios en las ventas calculando el Apalancamiento Operativo.","¿Cuál es el GAO de esta compañía dado su nivel actual de ventas y estructura de costes?","Halla el multiplicador del beneficio (Apalancamiento Operativo) para un aumento en las ventas."],m=p[n%p.length];o.push({id:y("AO",n),category:"Análisis de Costes",title:"Apalancamiento Operativo",question:m,data:{Ventas:l(t),"Costes Variables":l(i),"Costes Fijos":l(c)},solution:`AO = ${v(g,2)}`,explanation:`Fórmula: AO = MC / Beneficio
AO = ${a} / ${u} = ${v(g,4)}`,correctValue:g,valueType:"number"})}return o},j=e=>{const o=[],n=["liquidez","tesoreria","disponibilidad","garantia","endeudamiento","calidad_deuda","autonomia"];for(let t=0;t<e;t++){const i=n[t%n.length],a=d(100,1e4),s=d(10,50)*a,r=d(20,80)*a,c=d(30,100)*a,u=s+r+c,g=d(100,500)*a,p=u+g,m=Math.floor(p/d(120,250)*100),$=p-m,f=Math.floor(m*(d(30,80)/100));let h="",P="",M="",C={},x="",b=0;switch(i){case"liquidez":x="Ratio de Liquidez General";const V=["Calcula el Ratio de Liquidez General.","Determina la solvencia a corto plazo mediante el Ratio de Liquidez.","¿Cuál es la capacidad de la empresa para hacer frente a sus deudas a corto plazo con sus activos corrientes?"];h=V[t%V.length],C={"Activo Corriente":l(u),"Pasivo Corriente":l(f)},b=u/f,P=v(b,2),M=`Fórmula: AC / PC = ${u} / ${f} = ${b.toFixed(4)}`;break;case"tesoreria":x="Ratio de Tesorería";const F=["Calcula el Ratio de Tesorería (Acid Test).","Halla el coeficiente de tesorería (Acid Test) excluyendo las existencias.","Determina la capacidad de pago inmediata sin contar con la venta de stock (Acid Test)."];h=F[t%F.length],C={Realizable:l(r),Disponible:l(s),"Pasivo Corriente":l(f)},b=(r+s)/f,P=v(b,2),M=`Fórmula: (Realizable + Disponible) / PC = (${r} + ${s}) / ${f} = ${b.toFixed(4)}`;break;case"disponibilidad":x="Ratio de Disponibilidad";const E=["Calcula el Ratio de Disponibilidad.","¿Cuál es la disponibilidad inmediata de la empresa frente a sus deudas a corto plazo?","Determina el ratio más exigente de liquidez (Disponibilidad)."];h=E[t%E.length],C={Disponible:l(s),"Pasivo Corriente":l(f)},b=s/f,P=v(b,2),M=`Fórmula: Disponible / PC = ${s} / ${f} = ${b.toFixed(4)}`;break;case"garantia":x="Ratio de Garantía";const S=["Calcula el Ratio de Garantía.","Determina la solvencia total o garantía de la empresa frente a terceros.","¿Cuál es la distancia a la quiebra? Calcula el Ratio de Garantía."];h=S[t%S.length],C={"Activo Total":l(p),"Pasivo Total":l(m)},b=p/m,P=v(b,2),M=`Fórmula: Activo Total / Pasivo Total = ${p} / ${m} = ${b.toFixed(4)}`;break;case"endeudamiento":x="Ratio de Endeudamiento";const z=["Calcula el ratio de endeudamiento total.","Determina qué proporción de los recursos totales es financiada por deuda.","¿Cuál es el nivel de dependencia financiera externa (Ratio de Endeudamiento)?"];h=z[t%z.length],C={"Pasivo Total":l(m),"Patrimonio Neto":l($)},b=m/($+m),P=v(b,2),M=`Fórmula: Pasivo / (PN + Pasivo) = ${m} / (${$} + ${m}) = ${b.toFixed(4)}`;break;case"calidad_deuda":x="Ratio de Calidad de la Deuda";const L=["Calcula el ratio de Calidad de la Deuda.","¿Qué porcentaje de la deuda total vence a corto plazo? (Calidad de la Deuda).","Analiza la estructura de la deuda calculando su calidad."];h=L[t%L.length],C={"Pasivo Corriente":l(f),"Pasivo Total":l(m)},b=f/m,P=v(b,2),M=`Fórmula: Pasivo Corriente / Pasivo Total = ${f} / ${m} = ${b.toFixed(4)}`;break;case"autonomia":x="Ratio de Autonomía Financiera";const D=["Calcula el Ratio de Autonomía Financiera.","Determina la independencia financiera de la empresa."];h=D[t%D.length],C={"Patrimonio Neto":l($),"Pasivo Total":l(m)},b=$/m,P=v(b,2),M=`Fórmula: PN / Pasivo Total = ${$} / ${m} = ${b.toFixed(4)}`;break}o.push({id:y("RATIO",t),category:"Ratios Financieros y Patrimoniales",title:x,question:h,data:C,solution:P,explanation:M,correctValue:b,valueType:"number"})}return o},w=e=>{const o=[];for(let n=0;n<e;n++){const t=Math.random()>.5,i=d(100,1e3)*1e3,a=Math.floor(i*T(.4,.7)),s=Math.floor(i*T(.8,1.5)),r=Math.floor(s*T(.1,.25)),c=Math.floor(r*T(.1,.3)),g=Math.floor((r-c)*(1-.25));if(t){const p=r/i*100,m=["Calcula la Rentabilidad Económica (ROI).","Determina el rendimiento de los activos totales (ROI) antes de intereses e impuestos.","¿Cuál es la rentabilidad generada por el activo total de la empresa (ROI)?"],$=m[n%m.length];o.push({id:y("ROI",n),category:"Rentabilidad",title:"Cálculo del ROI",question:$,data:{"Activo Total":l(i),BAII:l(r)},solution:`${v(p)} %`,explanation:`Fórmula: (BAII / Activo) x 100
...
((${s} - ${a}) / ${a}) * 100 = ${v(r,2)} %`,correctValue:r,valueType:"percentage"})}else if(i==="commercial"){const r=s-a;o.push({id:y("MAR",t),category:"Márgenes",title:"Margen Comercial",question:"Calcula el margen comercial unitario (valor absoluto).",data:{"Precio Venta":l(s),"Coste Producto":l(a)},solution:l(r),explanation:`Pv - Cp = ${s} - ${a} = ${r}`,correctValue:r,valueType:"currency"})}else{const r=d(5e4,2e5),c=Math.floor(r*.6),u=r-c;o.push({id:y("MAR",t),category:"Márgenes",title:"Margen Industrial",question:"Calcula el Margen Industrial de la empresa.",data:{"Ingresos Totales":l(r),"Coste Productos Vendidos":l(c)},solution:l(u),explanation:`Ingresos - Coste Ventas = ${r} - ${c} = ${u}`,correctValue:u,valueType:"currency"})}}return o},_=()=>[...O(20),...k(20),...N(20),...j(20),...w(20),...B(20),...G(20),...H(20),...Q(20),...U(20)],K=[{id:"costes",title:"Análisis de Costes",content:`
  ...
  `},ae=e=>{const o=Object.entries(e.data).map(([i,a])=>`<div><span>${i}</span> <span>${a}</span></div>`).join(""),n=e.valueType==="percentage"?"Ej: 15.5":e.valueType==="currency"?"Ej: 5000":"Resultado",t=e.valueType==="percentage"?"%":e.valueType==="currency"?"€":"";return`
  ...
  `},oe=()=>`
    ...
    `,te=()=>{if(q==="theory")return oe();const e=Z();return`
    <main>
      <div class="content-wrapper">
        <header>
          <h2>${A}</h2>
  ...
  `},R=()=>{W.innerHTML=`
    ${ee()}
    ${te()}
  `,document.querySelectorAll("aside nav button[data-category]").forEach(e=>{e.addEventListener("click",o=>{A=o.target.dataset.category||"",q="exercises",R(),document.querySelector("main")?.scrollTo({top:0,behavior:"smooth"})})}),document.getElementById("btn-theory")?.addEventListener("click",()=>{q="theory",R(),document.querySelector("main")?.scrollTo({top:0,behavior:"smooth"})})};Y();
